            ...
            <div style={{ flexGrow: 1, display: 'flex', height: 'calc(100vh - 70px)', overflow: 'hidden' }}>
                ...
                <div style={{
                    ...
                    {[
                        ...
                        //     tooltip: isAudioAvailable ? 'View Audio Files' : 'No Audio for this page'
                        // },
                        // {
                        //     name: 'All Audios',
                        //     icon: faHeadphones,
                        //     panel: 'allAudios',
                        //     // افترض أن 'resource' هو الكائن الذي يحتوي على بيانات الكتاب
                        //     disabled: !(resource?.pageAudios?.length > 0),
                        //     onClick: (tool) => {
                        //         if (resource?.pageAudios?.length > 0) {
                        //             setActivePanel(prev => prev === tool.panel ? null : tool.panel);
                        //         }
                        //     },
                        //     // لتمييزها عن أيقونة الصوت الخاصة بالصفحة
                        //     color: (resource?.pageAudios?.length > 0) ? '#1f3e72' : '#ddd'
                        // },
                        ...
                        //     tooltip: isVideoAvailable ? 'View Video Files' : 'No Video for this page'
                        // },
                        // {
                        //     name: 'All Videos',
                        //     icon: faVideo,
                        //     panel: 'allVideos',
                        //     disabled: !(resource?.pageVideos?.length > 0),
                        //     onClick: (tool) => {
                        //         if (resource?.pageVideos?.length > 0) {
                        //             setActivePanel(prev => prev === tool.panel ? null : tool.panel);
                        //         }
                        //     },
                        //     color: (resource?.pageVideos?.length > 0) ? '#1f3e72' : '#ddd'
                        // },
                    ].map((tool) => (
                        <SidebarItem
                            key={tool.name}
                ...
                {['menu', 'bookmarks', 'activity'].includes(activePanel) && (
                    <RenderSidePanel />
                )}
